...
def evaluate_dice(emoji: str, value: int) -> Tuple[float, str]:
    ...
    Награды существенно снижены (анти-имбаланс).
    Для ⚽ маппинг сдвинут на 1 относительно визуала анимации
    (value=5 → гол, value=4 → штанга, ниже → мимо).
    """
    if emoji == "🎲":  # 1–6
        if value == 6:
            return 2.5, "🎲 Шестёрка! x2.5 — отличный бросок!"
        if value == 5:
            return 1.5, "🎲 Пятёрка! x1.5"
        if value == 4:
            return 1.0, "🎲 Четвёрка — возврат ставки"
        return 0.0, f"🎲 Выпало {value}… ставка сгорела"

    if emoji == "🎯":  # 1–6 (6 = в яблочко)
        if value == 6:
            return 3.0, "🎯 В яблочко!!! x3 — идеальный выстрел!"
        if value == 5:
            return 1.5, "🎯 Почти центр! x1.5"
        if value == 4:
            return 1.0, "🎯 Хороший бросок — возврат ставки"
        return 0.0, f"🎯 Промах ({value})… ставка сгорела"

    if emoji == "🎳":  # 1–6 (6 = страйк)
        if value == 6:
            return 2.5, "🎳 СТРАЙК!!! x2.5 — все кегли сбиты!"
        if value == 5:
            return 1.5, "🎳 Почти страйк! x1.5"
        if value == 4:
            return 1.0, "🎳 Четыре кегли — возврат ставки"
        return 0.0, f"🎳 Всего {value}… ставка сгорела"

    if emoji == "🏀":  # 1–5 (4–5 = попал)
        if value == 5:
            return 2.0, "🏀 Красивый данк! x2"
        if value == 4:
            return 1.5, "🏀 Мяч в кольце! x1.5"
        if value == 3:
            return 1.0, "🏀 Почти… возврат ставки"
        return 0.0, f"🏀 Мимо ({value})… ставка сгорела"

    if emoji == "⚽":  # 1–5, сдвиг на 1 относительно визуала
        # Визуально: value=5 → гол, value=4 → штанга, 1–3 → мимо
        if value == 5:
            return 2.0, "⚽ Гол!!! x2 — красивый удар!"
        if value == 4:
            return 1.0, "⚽ Штанга… возврат ставки"
        return 0.0, f"⚽ Мимо ({value})… ставка сгорела"

    if emoji == "🎰":  # 1–64
        # 64 = три семёрки (джекпот) — очень редко
        if value == 64:
            return 5.0, "🎰 JACKPOT 777!!! x5 — невероятно!"
        # Две семёрки (примерно 16, 32, 48)
        if value in (16, 32, 48):
            return 2.0, "🎰 Две семёрки! x2"
        # Три одинаковых (кроме семёрок)
        if value in (1, 22, 43):
            return 1.5, "🎰 Три одинаковых! x1.5"
        # Высокие значения — небольшой выигрыш / возврат
        if value >= 50:
            return 1.2, "🎰 Хорошая комбинация — x1.2"
        if value >= 30:
            return 1.0, "🎰 Неплохо — возврат ставки"
